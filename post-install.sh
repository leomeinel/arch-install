...
doas sh -c '{
  ...
  echo "fi"
  echo "/usr/bin/cryptsetup luksOpen --type luks1 \"/dev/disk/by-uuid/$DISK2P1_UUID\" boot_crypt_backup"
  ...
  echo "/usr/bin/umount -AR /boot"
  echo "/usr/bin/umount -AR /efi"
  echo "/usr/bin/cryptsetup luksOpen --type luks1 \"/dev/disk/by-uuid/$DISK1P1_UUID\" boot_crypt"
