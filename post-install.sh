...
doas sh -c '{
  ...
  DISK1P2_UUID="$(lsblk -rno LABEL,MOUNTPOINT,UUID | grep "EFI /efi" | sed "s/EFI \/efi//" | tr -d "[:space:]")"
  DISK2P2_UUID="$(lsblk -rno LABEL,MOUNTPOINT,UUID | grep "EFI  " | sed "s/EFI  //" | tr -d "[:space:]")"
  echo "#!/bin/sh"
  echo ""
  ...
  echo "fi"
  echo "/usr/bin/cryptsetup luksClose --type luks1 \"$DISK1P1\""
  echo "if /usr/bin/mountpoint -q /efi"
  echo "then"
  echo "  /usr/bin/umount -AR /efi"
  echo "fi"
  echo "/usr/bin/cryptsetup luksOpen --type luks1 \"$DISK2P1\" boot_crypt_backup"
  ...
  echo "/usr/bin/umount -AR /boot"
  echo "/usr/bin/umount -AR /efi"
  echo "/usr/bin/cryptsetup luksClose --type luks1 \"$DISK2P1\""
  echo "/usr/bin/cryptsetup luksOpen --type luks1 \"$DISK1P1\" boot_crypt"
