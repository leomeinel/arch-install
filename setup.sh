...
PARAMETERS="rd.auto=1 rd.luks.uuid=${MD0UUID} rd.luks=1 rd.lvm=1 rd.md=1 root=/dev/mapper/vg0-lv0 rd.vconsole.unicode rd.vconsole.keymap=${KEYMAP} loglevel=3 bgrt_disable audit=1 audit_backlog_limit=8192 lsm=landlock,lockdown,yama,integrity,apparmor,bpf iommu=pt zswap.enabled=0 lockdown=integrity module.sig_enforce=1"
